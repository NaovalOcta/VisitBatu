...
@section('content')
    <div class="w-full max-w-md bg-white dark:bg-night-800 rounded-3xl shadow-2xl dark:shadow-night-950/50 overflow-hidden p-8 md:p-12 m-4 transition-colors duration-300">
        <div class="text-center">
            ...
            <h1 class="font-serif text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-4">Verifikasi Email Anda</h1>

            <p class="text-gray-600 dark:text-white/60 mb-6 leading-relaxed">
                Terima kasih sudah mendaftar! Sebelum memulai, mohon verifikasi email Anda dengan mengklik link yang telah
                kami kirimkan ke:
            </p>

            <p class="text-primary-700 dark:text-primary-400 font-bold mb-6">{{ Auth::user()->email }}</p>
            ...
            <div class="space-y-4">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-gray-500 dark:text-white/50 font-medium hover:text-gray-700 dark:hover:text-white transition text-sm">
                        Logout
                    </button>
                </form>
            </div>

            <p class="mt-8 text-xs text-gray-400 dark:text-white/50">
                Tidak menerima email? Cek folder spam Anda atau hubungi
                <a href="{{ route('contact_page') }}" class="text-primary-600 dark:text-primary-400 hover:underline">support kami</a>.
            </p>
        </div>
    </div>
@endsection
