...
@section('content')
    <div class="bg-primary-900 dark:bg-slate-950 pt-32 pb-24 text-center text-white px-4 relative overflow-hidden transition-colors duration-300">
    ...
    <div class="bg-white dark:bg-slate-900 min-h-screen pb-20 -mt-10 relative z-20 rounded-t-[3rem] transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 pb-12 border-b border-gray-100 dark:border-slate-800">
                <div class="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto no-scrollbar">
                    <button
                        class="px-6 py-2 rounded-full bg-primary-900 dark:bg-slate-100 text-white dark:text-slate-900 font-bold text-sm whitespace-nowrap transition-colors duration-300">Semua</button>
                    <button
                        class="px-6 py-2 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 font-medium text-sm hover:bg-gray-200 dark:hover:bg-slate-700 transition whitespace-nowrap">Kuliner</button>
                    <button
                        class="px-6 py-2 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 font-medium text-sm hover:bg-gray-200 dark:hover:bg-slate-700 transition whitespace-nowrap">Tips
                        Wisata</button>
                    <button
                        class="px-6 py-2 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 font-medium text-sm hover:bg-gray-200 dark:hover:bg-slate-700 transition whitespace-nowrap">Itinerary</button>
                </div>

                <div class="relative w-full md:w-72">
                    <input type="text" placeholder="Cari artikel..."
                        class="w-full pl-10 pr-4 py-2 border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-200 placeholder-gray-400 dark:placeholder-slate-500 rounded-full text-sm focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors duration-300">
                    <svg class="w-4 h-4 text-gray-400 dark:text-slate-500 absolute left-3.5 top-2.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                ...
                @foreach ($posts as $post)
                    ...
                    <article class="flex flex-col group h-full">
                        <div class="rounded-2xl overflow-hidden h-64 mb-6 shadow-md relative">
                            ...
                            @if ($post->trip)
                                <div
                                    class="absolute top-4 right-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-teal-700 dark:text-teal-400 shadow-sm flex items-center gap-1">
                        ...
                        <div class="flex-1 flex flex-col">
                            ...
                            <div
                                class="flex items-center gap-2 text-xs text-gray-500 dark:text-slate-400 mb-3 uppercase tracking-wide font-bold">
                            ...
                            {{-- Judul Postingan --}}
                            <h3
                                class="font-serif text-2xl font-bold text-gray-900 dark:text-slate-100 mb-3 leading-snug group-hover:text-primary-700 dark:group-hover:text-primary-400 transition">
                                <a href="{{ route('blog.show', $post->slug) }}">{{ $post->title }}</a>
                            </h3>

                            {{-- Cuplikan Isi --}}
                            <p class="text-gray-600 dark:text-slate-400 line-clamp-3 mb-4 font-light text-sm">
            ...
            <div class="mt-20 text-center">
                <button
                    class="px-8 py-3 bg-white dark:bg-slate-800 border-2 border-primary-900 dark:border-primary-400 text-primary-900 dark:text-primary-400 font-bold rounded-full hover:bg-primary-900 hover:text-white dark:hover:bg-primary-400 dark:hover:text-slate-900 transition shadow-lg shadow-gray-200 dark:shadow-none">
