        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    {{-- Main Image --}}
                    <div class="bg-white dark:bg-slate-800 p-2 mt-5 rounded-4xl shadow-xl transition-colors duration-300">
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-slate-700/50 transition-colors duration-300">
                        <h3 class="font-serif text-2xl font-bold text-gray-900 dark:text-slate-100 mb-6 flex items-center gap-3">
                            <span class="w-8 h-1 bg-primary-600 rounded-full block"></span>
                            Tentang Destinasi
                        </h3>

                        <div class="prose prose-lg text-gray-600 dark:text-slate-400 leading-relaxed font-sans max-w-none">
                    ...
                    {{-- 3. REVIEW SECTION --}}
                    <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-slate-700/50 transition-colors duration-300">
                        <h3 class="font-serif text-2xl font-bold text-gray-900 dark:text-slate-100 mb-8 flex items-center gap-3">
                            <span class="w-8 h-1 bg-accent-500 rounded-full block"></span>
                            Ulasan Pengunjung
                        </h3>

                        {{-- Rating Stats --}}
                        <div class="flex items-center gap-6 mb-10 pb-10 border-b border-gray-100 dark:border-slate-700/50">
                            <div class="text-center">
                                <div class="text-5xl font-bold text-gray-900 dark:text-slate-100 mb-1">
                                    {{ number_format($trip->average_rating, 1) }}</div>
                                <div class="flex text-accent-500 mb-1">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 {{ $i <= round($trip->average_rating) ? 'fill-current' : 'text-gray-200 dark:text-slate-600' }}"
                                ...
                                <div class="text-sm text-gray-400 dark:text-slate-500 font-medium">{{ $trip->reviews->count() }} Ulasan</div>
                            </div>
                        </div>

                        {{-- Review Form (Hanya untuk yang sudah login) --}}
                        @auth
                            <div class="mb-12 bg-gray-50 dark:bg-slate-900/50 rounded-2xl p-6 border border-gray-100 dark:border-slate-700/50" x-data="{ rating: 0, isLoading: false }">
                                <h4 class="font-bold text-gray-900 dark:text-slate-100 mb-4">Berikan Ulasan Anda</h4>
                                <form action="{{ route('reviews.store', $trip->slug) }}" method="POST"
                                    ...
                                    <div class="flex items-center gap-2 mb-4">
                                        <span class="text-sm text-gray-600 dark:text-slate-400 mr-2">Rating:</span>
                                        <template x-for="i in 5">
                                            <button type="button" @click="rating = i"
                                                class="focus:outline-none transition transform hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8"
                                                    :class="i <= rating ? 'text-accent-500 fill-current' : 'text-gray-300 dark:text-slate-600'"
                                                    viewBox="0 0 20 20" fill="currentColor">
                                                    <path
                                                        d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                                </svg>
                                    ...
                                    <div class="mb-4">
                                        <textarea name="comment" rows="3" required
                                            class="w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 focus:ring-2 focus:ring-primary-500 outline-none text-gray-700 dark:text-slate-200 transition"
                                ...
                                <div class="flex gap-4">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between mb-1">
                                            <h5 class="font-bold text-gray-900 dark:text-slate-100">{{ $review->user->name }}</h5>
                                            <span
                                        ...
                                        <div class="flex text-accent-500 mb-2">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    class="h-4 w-4 {{ $i <= $review->rating ? 'fill-current' : 'text-gray-200 dark:text-slate-600' }}"
                                                    viewBox="0 0 20 20" fill="currentColor">
                                                    <path
                                                        d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                                </svg>
                                        ...
                                        <p class="text-gray-600 dark:text-slate-400 leading-relaxed">{{ $review->comment }}</p>
                                    </div>
                                </div>
                            @empty
                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-28 space-y-6">
                        ...
                        <div
                            class="bg-white dark:bg-slate-800 rounded-3xl p-8 mt-5 shadow-lg dark:shadow-slate-900/50 border border-gray-100 dark:border-slate-700/50 relative overflow-hidden group transition-colors duration-300">
                            <div
                            ...
                            <p class="text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-2 relative z-10">Harga Tiket Masuk</p>
                            <div class="flex items-baseline gap-1 mb-6 relative z-10">
                                <span class="text-sm text-gray-400 dark:text-slate-500 font-bold">IDR</span>
                                <span
                            ...
                            <div class="space-y-4 mb-8">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        <p class="text-xs text-gray-400 dark:text-slate-500 font-bold uppercase">Jam Buka / Durasi</p>
                                        <p class="text-gray-800 dark:text-slate-200 font-medium">{{ $trip->duration }}</p>
                                    </div>
                                </div>
                                <div class="flex items-start gap-4">
                                    <div
                                    ...
                                    <div>
                                        <p class="text-xs text-gray-400 dark:text-slate-500 font-bold uppercase">Lokasi</p>
                                        <p class="text-gray-800 dark:text-slate-200 font-medium">{{ $trip->location }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <a href="{{ route('contact_page') }}"
                                class="block w-full py-4 bg-gray-900 dark:bg-slate-100 text-white dark:text-gray-900 font-bold text-center rounded-xl hover:bg-primary-600 dark:hover:bg-primary-400 dark:hover:text-white transition shadow-lg transform hover:-translate-y-1">
                                Hubungi Kami
                            </a>
                        ...
                        @if ($trip->latitude && $trip->longitude)
                            <div class="bg-white dark:bg-slate-800 rounded-3xl p-4 shadow-lg dark:shadow-slate-900/50 border border-gray-100 dark:border-slate-700/50 overflow-hidden transition-colors duration-300">
                                <h4 class="font-serif text-lg font-bold text-gray-900 dark:text-slate-100 mb-4 px-2">Lokasi di Peta</h4>
                                ...
                                <div class="mt-4 px-2 flex justify-between items-center">
                                    <span class="text-[10px] text-gray-400 dark:text-slate-500 font-bold uppercase tracking-widest">Interactive
                                        Map</span>
