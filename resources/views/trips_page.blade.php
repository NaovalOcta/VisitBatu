...
@section('content')
    <div class="relative bg-primary-900 pt-32 pb-20 overflow-hidden">
        ...
        <div class="relative z-10 text-center max-w-4xl mx-auto px-4">
            <span class="text-accent-400 font-bold tracking-widest uppercase text-sm mb-4 inline-block">Discover Nature</span>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row gap-10">
                ...
                <form action="{{ route('trips.index') }}" method="GET" id="filterForm" class="w-full lg:w-1/4">
                    ...
                    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 sticky top-28">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <a href="{{ route('trips.index') }}" class="text-xs text-primary-700 font-bold hover:underline">Reset</a>
                        </div>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <div class="space-y-3">
                                    @php
                                        $categories = ['Wisata Alam', 'Theme Park', 'Edukasi', 'Kuliner', 'Hotel & Resort'];
                                    @endphp
                                    @foreach ($categories as $cat)
                                        <label class="flex items-center gap-3 cursor-pointer group">
                                            ...
                                            <span class="text-gray-600 group-hover:text-primary-700 transition">{{ $cat }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="pt-6 border-t border-gray-100">
                                <label class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 block">Maksimal Harga</label>
                            ...
                            <div class="pt-4">
                                <button type="submit" class="w-full py-3 bg-gray-900 text-white rounded-xl font-bold text-sm hover:bg-primary-700 transition shadow-lg">
                                    Terapkan Filter
                                </button>
                            </div>
                        </div>
                ...
                <div class="w-full lg:w-3/4">
                    <div class="flex justify-between items-center mb-6">
                        <p class="text-gray-500 text-sm">Menampilkan <span class="font-bold text-gray-900">{{ $trips->total() }}</span> destinasi</p>

                        <select onchange="document.getElementById('hiddenSortInput').value = this.value; document.getElementById('filterForm').submit();"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        @forelse($trips as $trip)
                            <div class="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col h-full">
                                ...
                                <div class="p-6 flex flex-col flex-grow">
                                    ...
                                    <p class="text-gray-500 mb-6 line-clamp-2 text-sm flex-grow">{{ $trip->description }}</p>
                                    <div class="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto">
                                        <div>
                                            <p class="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Tiket Masuk</p>
                                            <p class="text-xl font-bold text-primary-700">Rp {{ number_format($trip->price) }}</p>
                                        </div>
