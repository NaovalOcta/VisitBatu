                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    <div class="h-12 w-16 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100 border border-gray-200 relative">
                                        @if($post->image)
                                    ...
                                    <div>
                                        ...
                                        {{-- Link ke Show (Preview) --}}
                                        <a href="{{ route('user.posts.show', $post->id) }}" class="text-[10px] text-teal-600 hover:underline font-bold">
                                            Lihat Preview ↗
                                        </a>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    {{-- Tombol Hapus --}}
                                    <form action="{{ route('user.posts.destroy', $post->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus cerita ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-500 text-xs mb-4">Mulai bagikan pengalaman perjalanan Anda sekarang.</p>
                                    <a href="{{ route('user.posts.create') }}" class="text-teal-600 hover:text-teal-700 text-xs font-bold underline">
                                        Buat cerita pertama
                                    </a>
                                </div>
                            </td>
